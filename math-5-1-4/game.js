...
function getRandomProblem() {
  return PROBLEM_PACK.problems[randInt(0, PROBLEM_PACK.problems.length - 1)];
}
...
function checkAdventureAnswer() {
  ...
  stopAdvTimer();
  const correct = input === gameState.currentQuestion.answer;
  const elapsed = (Date.now() - gameState.advQuestionStartTime) / 1000;
...
function showAdvHint() {
  ...
  const hint = `정답은 "${gameState.currentQuestion.answer}" 입니다!`;
