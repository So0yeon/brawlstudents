...
function refreshCharLabels() {
  ...
  document.querySelectorAll('.character-card').forEach(card => {
    ...
    card.querySelector('.char-labels').innerHTML = '';
  });
  ...
  document.querySelectorAll('.character-card').forEach(card => {
    ...
    const badges = [];
    if (id === p1Id) badges.push('<span class="badge-p1">P1</span>');
    if (id === p2Id) badges.push('<span class="badge-p2">P2</span>');
    if (badges.length > 0) {
      labels.innerHTML = badges.join('');
      card.classList.add(id === p1Id ? 'selected-p1' : 'selected-p2');
      // 같은 카드면 p1 스타일 우선, p2 뱃지도 추가
      if (id === p1Id && id === p2Id) {
        card.classList.add('selected-p1', 'selected-p2-same');
      }
    }
  });
}
