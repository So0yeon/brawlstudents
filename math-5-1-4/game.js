...
const CHARACTERS = [
  { id: 'shelly', emoji: '🦸‍♀️', name: '쉘리',
    power: '⚡ 정답시 +5데미지',
    desc: '정답을 맞힐 때마다 기본 데미지에 +5가 추가돼요. 꾸준히 맞히면 강력해요!',
    ability: 'bonusDamage', bonus: 5 },
  { id: 'colt', emoji: '🤠', name: '콜트',
    power: '⏱ 풀이시간 +3초',
    desc: '문제당 풀이시간이 15초→18초로 늘어나요. 어려운 문제도 여유롭게 풀 수 있어요!',
    ability: 'extraTime', timeBonus: 3 },
  { id: 'nita', emoji: '🐻', name: '니타',
    power: '💚 정답시 HP+5 회복',
    desc: '정답을 맞힐 때마다 HP가 5씩 회복돼요. 배틀 모드에선 점수 +15 보너스!',
    ability: 'heal', healAmount: 5 },
  { id: 'leon', emoji: '🥷', name: '레온',
    power: '🛡 3연속 정답시 보호막',
    desc: '3연속 정답을 맞히면 보호막이 생겨요. 다음 오답의 피해를 한 번 막아줘요!',
    ability: 'shield' },
  { id: 'spike', emoji: '🌵', name: '스파이크',
    power: '🔥 오답에도 콤보 유지',
    desc: '틀려도 콤보가 끊기지 않아요. 콤보 보너스를 계속 유지할 수 있어요!',
    ability: 'comboKeep' },
  { id: 'crow', emoji: '🦅', name: '크로우',
    power: '🗡 5초내 정답시 +10데미지',
    desc: '5초 안에 정답을 맞히면 데미지 +10이 추가돼요. 빠르게 풀수록 강력해요!',
...
function resolveRound() {
  ...
  // P1 데미지 → P2 HP 감소
  gameState.pvpHP[1] = Math.max(0, gameState.pvpHP[1] - dmg1);
  // P2 데미지 → P1 HP 감소
  gameState.pvpHP[0] = Math.max(0, gameState.pvpHP[0] - dmg2);

  // 레온 보호막: 3연속 정답 시 이번 라운드 받은 데미지 1회 무효
  if (gameState.p1Character.ability === 'shield' &&
      gameState.pvpCombos[0] > 0 && gameState.pvpCombos[0] % GAME_CONFIG.shieldAbilityTrigger === 0) {
    gameState.pvpHP[0] = Math.min(100, gameState.pvpHP[0] + dmg2);
    showFeedback('🛡 P1 레온 보호막 발동!', 'correct');
  }
  if (gameState.p2Character.ability === 'shield' &&
      gameState.pvpCombos[1] > 0 && gameState.pvpCombos[1] % GAME_CONFIG.shieldAbilityTrigger === 0) {
    gameState.pvpHP[1] = Math.min(100, gameState.pvpHP[1] + dmg1);
  }
